...
class inference():
    ...
    def get_nearerst_images(self, face_set, voice_set):
        ...
        classA, classB = face_set_labels_to_indices[1], face_set_labels_to_indices[6]

        transform = transforms.Compose(
            [transforms.ToPILImage()])

        for i in sorted(face_set_labels_to_indices):
            emotion_class = face_set_labels_to_indices[i]
        # get 10 nearest neighbors for a neutral image
        for img_type in [classA]:
            ...
            torchvision.utils.save_image(nearest_imgs,'./utils/Face_emotions.png',nrow=5, normalize=True, range=(-1,1), scale_each=False)
        ...
        for img_type in [classA]:
            ...
            nearest_imgs = [face_set[i][0] for i in indices[0]]
            print("nearest images")
            imshow_face_figure(torchvision.utils.make_grid(nearest_imgs))
        i = 1
